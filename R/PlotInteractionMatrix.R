...
#'
#' Visualizes a spatial interaction matrix using a heatmap,
#' where rows represent focal clusters and columns represent neighbor clusters.
#' Each row is scaled using z-scores to highlight
#' relative enrichment patterns across neighbor types.
#' This is useful for detecting spatial proximity patterns
...
#' @param angle_x_label Angle (in degrees) to rotate x-axis labels.
#'                      Default is `45`.
#' @param title Title for the heatmap.
#'
#' @return A `ggplot` object representing the row-scaled heatmap
...
PlotInteractionMatrix <- function(
    ...
    # Keep raw orders
    df_long$Focal <- factor(df_long$Focal,
        levels = rownames(interaction_matrix)
    )
    df_long$Neighbor <- factor(df_long$Neighbor,
        levels = colnames(interaction_matrix)
    )

    # Plot
    ggplot2::ggplot(df_long, ggplot2::aes(
        x = .data$Neighbor, y = .data$Focal,
        fill = .data$Zscore
    )) +
        ggplot2::geom_tile(color = "white") +
        ggplot2::scale_fill_gradient2(
            low = low_color, mid = mid_color,
