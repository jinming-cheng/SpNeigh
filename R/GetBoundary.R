...
#' @param k The number of nearest neighbors to use for computing
#'          local distances. Default is 5.
#' @param distance_cutoff A numeric threshold on the average distance
...
RemoveOutliers <- function(coords, k = 5, distance_cutoff = 30) {
    ...
    coords <- dplyr::filter(coords, .data$mean_knn_dist < distance_cutoff)
...
#'
#' This function extracts a spatial coordinate data frame from either
#' a Seurat object or a user-supplied data frame.
#' When a Seurat object is provided, it retrieves tissue coordinates (`x`, `y`)
#' and optionally the cluster identities (e.g., `seurat_clusters`).
#' When a data frame is provided, it is returned as-is after checking that
...
#'
#' @return A data frame with columns `x`, `y`, `cell`, and optionally
#'         `cluster`, representing spatial coordinates and cluster assignments.
#'
#' @export
#' @examples
#' # Load coordinates
...
ExtractCoords <- function(data = NULL, extract_cluster = TRUE) {
    # If data is a Seurat object, extract the coordinates
    ...
    if (is(data, "Seurat")) {
        ...
        # A data frame includes coordinates and cluster information
...
#' @param one_cluster The cluster ID (numeric or character) to extract
#'                    the boundary for.
...
#'                     `subregion_method = "kmeans"`. Default is 3.
#'
#' @return A data frame containing boundary points with
#'         columns `x`, `y`, and `region_id`.
#'         If `multi_region = TRUE`, multiple boundaries are returned
#'         and labeled by `region_id`.
...
GetBoundary <- function(
    ...
    if (!(one_cluster %in% sp_coords$cluster)) {
        stop(
            "`one_cluster` must exist in the `cluster`",
    ...
    clean_df <- RemoveOutliers(coords_sub[, c("x", "y")],
        k = k,
    ...
    if (nrow(clean_df) == 0) {
        stop(
            "All cells were removed as outliers.",
            " Consider increasing `distance_cutoff` in",
            " the `RemoveOutliers` function to retain more cells."
        )
    }
    ...
    # If a cluster (or cell population) has multiple regions,
    # obtain multiple boundaries
    if (multi_region) {
        if (subregion_method == "dbscan") {
            # automatically detect the regions using dbscan method
            db <- dbscan::dbscan(clean_df[, c("x", "y")],
                eps = eps, minPts = minPts
            )
            clean_df$region_id <- db$cluster
            # remove noise
            clean_df <- dplyr::filter(clean_df, .data$region_id != 0)
        } else if (subregion_method == "kmeans") {
            # alternatively, manually specify the region number
            # using kmeans method
        ...
        boundary_list <- lapply(unique(clean_df$region_id), function(reg_id) {
            ...
            hull_coords <- concaveman::concaveman(as.matrix(
                region_df[, c("x", "y")]
            ))
        ...
        if (nrow(all_boundaries) == 0) {
            stop(
                "No subregions were detected.",
                " Please adjust the parameters `eps` and `minPts`",
                " for the DBSCAN method, or `n_subregions` for",
                " the k-means method when `multi_region = TRUE`.",
                " Alternatively, set `multi_region = FALSE` to",
                " disable subregion detection."
            )
        }
...
#'
#' @param boundary A data frame of boundary points, typically returned
#'                 by `GetBoundary()`.
#'                 Must contain columns `x`, `y`, and `region_id`.
#'
#' @return An `sf` object containing one or more valid `POLYGON` geometries
#'         with a `region_id` column.
#'
#' @export
#'
#' @examples
...
BuildBoundaryPoly <- function(boundary = NULL) {
    if (is.null(boundary) ||
        !all(c("x", "y", "region_id") %in% colnames(boundary))) {
...
#'
#' Extracts the vertex coordinates (`x`, `y`) of boundary geometries
#' from an `sf` object containing `POLYGON` or `LINESTRING` features.
#' This is useful for recovering the original boundary points from
#' smoothed or labeled polygonal regions.
#'
#' @param boundary_poly An `sf` object containing only `POLYGON` or
#'                      `LINESTRING` geometries.
#'                      Must include a `region_id` column for labeling
#'                      subregions.
#'
#' @return A data frame with columns `x`, `y`, and `region_id`,
#'         containing the vertex coordinates of the boundary geometries.
#'
#' @export
#'
#' @examples
...
#' head(boundary_pts)
#'
BoundaryPolyToPoints <- function(boundary_poly = NULL) {
    ...
    if (!all(geom_type %in% valid_types)) {
        stop(
            "All geometries in the input must be",
            " either POLYGON or LINESTRING."
        )
    }
...
#' Generate an outer or inner boundary polygon by buffering
...
#' polygons may collapse, become invalid, or disappear entirely if the
#' buffer width exceeds the shape's interior size.
#'
#'
#' @param boundary A data frame of boundary points
#'                 (with columns `x`, `y`, `region_id`) or an `sf` object.
#' @param dist A numeric value specifying the buffer distance in spatial units.
#'             Use positive values to expand (outer boundary),
#'             and negative values to shrink (inner boundary). Default is 100.
#'
#' @return An `sf` object of expanded outer boundary polygons with the
...
#'
#' Computes an inward-shifted (contracted) boundary by applying a
...
#'
#' Be careful: if the shrinkage distance is too large, the resulting geometry
#' may become invalid or disappear entirely, especially for narrow
#' or irregular shapes.
#'
#' @param boundary A data frame of boundary points
#'                 (with columns `x`, `y`, `region_id`)
#'                 or an `sf` object of `POLYGON` geometries.
#' @param dist A positive numeric value specifying how far inward to
#'             shrink the boundary.
#'             This is automatically converted to a negative buffer distance.
#'             Default is 50.
#'
#' @return An `sf` object containing the inward-shrunk polygons,
#'         one per `region_id`.
#'
...
GetInnerBoundary <- function(boundary = NULL, dist = 50) {
    ...
    if (any(sf::st_is_empty(inner))) {
        warning(
            "Some inner boundaries may have collapsed",
            " or disappeared due to excessive shrinkage."
        )
    }
...
#' @param outer_boundary Optional `sf` object containing
#'                       buffered (outer) boundary polygons.
#'                       If not provided, it will be automatically computed
#'                       using `GetOuterBoundary()`.
#' @param ...  Additional arguments passed to `GetOuterBoundary()`
...
GetRingRegion <- function(boundary = NULL, outer_boundary = NULL, ...) {
    ...
    ring_geoms <- purrr::map2(
        outer_boundaries$geometry,
...
#'
#' This function takes an sf POLYGON object and two anchor points
#' (or infers them) to split the polygon boundary into
#' two LINESTRING edge segments.
#'
#' @param boundary_poly An sf POLYGON object.
#' @param pt1 A numeric vector of length 2 (x, y) or NULL.
#'            If NULL, the leftmost boundary point is used.
#' @param pt2 A numeric vector of length 2 (x, y) or NULL.
#'            If NULL, the rightmost boundary point is used.
#'
#' @return An sf object with two LINESTRING features and a 'region_id' column
...
#'
#' @export
SplitBoundaryPolyByAnchor <- function(
    boundary_poly = NULL,
    pt1 = NULL,
    pt2 = NULL) {
    if (!inherits(boundary_poly, "sf") ||
        !all(sf::st_geometry_type(boundary_poly) == "POLYGON")) {
        stop("Input must be an sf object with POLYGON geometry.")
    }

    coords <- sf::st_coordinates(sf::st_geometry(boundary_poly)[[1]])[, c(1, 2)]
    ...
    if (!is.numeric(pt1) || length(pt1) != 2 ||
        !is.numeric(pt2) || length(pt2) != 2) {
        stop(
            "pt1 and pt2 must each be a numeric vector of",
            " length 2 (x, y) or NULL."
        )
    }
    ...
    n <- nrow(coords)
    path1 <- coords[seq(from = idx1, to = idx2), ]
    path2 <- coords[c(seq(from = idx2, to = n), seq_len(idx1)), ]
