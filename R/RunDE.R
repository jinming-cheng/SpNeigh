...
#' `limma` linear modeling framework. Supports optional observation-level weights
#' (e.g., spatial weights) and filters genes by minimum expression threshold across groups.
#'
#' @param exp_mat A normalized gene expression matrix (genes x cells), either a `matrix` or `dgCMatrix`.
#'                Typically log-normalized counts, e.g., from a Seurat object.
#' @param cells_reference A character vector of cell IDs to use as the reference (baseline) group.
#' @param cells_target A character vector of cell IDs to use as the target (comparison) group.
#' @param weights Optional numeric vector of observation-level weights. Must be named with cell IDs and
#'                match the length of `cells_reference + cells_target`.
#' @param adj_p.value Adjusted p-value threshold for reporting differentially expressed genes. Default is `0.05`.
#' @param min.pct Minimum proportion of cells expressing the gene in either group (values between 0 and 1).
#'                Genes not meeting this threshold are excluded before testing. Default is `0`.
#'
#' @return A data frame with differentially expressed genes, sorted by absolute log fold change.
#'         Includes columns:
#' \describe{
...
#'
#' # Subset cells from cluster 0 and 6
...
RunLimmaDE <- function(exp_mat = NULL,
  ...
  if (!is.null(weights)) {
    if (is.null(names(weights)) || length(weights) != length(all_cells)) {
      stop("'weights' must be a named numeric vector with the same length as all selected cells.")
    }
...
#'
#' Performs spatially-aware differential expression (DE) analysis by modeling gene expression
#' as a smooth function of a continuous spatial covariate (e.g., distance to a boundary or centroid).
#' Natural spline basis functions are used to capture non-linear trends in expression relative to spatial distance.
#' This method is suitable for identifying genes whose expression varies continuously across spatial structures.
#'
#' @inheritParams RunLimmaDE
#' @inheritParams SplineDesign
#' @param cell_ids A character vector of cell IDs (column names of `exp_mat`) used for DE analysis.
#' @param spatial_distance A named numeric vector containing the spatial distance (or weights) for each cell.
#'                         Must be the same length as `cell_ids`. Scaled distances are recommended.
#'
#' @return A data frame of differentially expressed genes, including:
#' \describe{
#'   \item{AveExpr, F, P.Value, adj.P.Val}{limma differential expression outputs}
#'   \item{Z1, Z2, Z3}{Spline coefficients (Z1 typically corresponds to linear trend)}
...
#'
#' # Run spatial DE
#' result <- RunSpatialDE(exp_mat = logNorm_expr, cell_ids = cells_c0, spatial_distance = weights)
#' head(result)
#'
RunSpatialDE <- function(exp_mat = NULL,
  ...
  if (!is.null(spatial_distance)) {
    if (is.null(names(spatial_distance)) || length(spatial_distance) != length(all_cells)) {
      stop("'spatial_distance' must be a named vector, and its length must match the number of selected cells.")
    }
  }
  ...
  fit <- limma::eBayes(fit)
  tab <- limma::topTable(fit, coef = 2:(df + 1), number = Inf, p.value = adj_p.value)
...
#'
#' Constructs an orthonormal design matrix from a numeric covariate (e.g., spatial distance)
#' using a natural cubic spline basis. The output matrix can be used in linear modeling
#' to capture smooth, non-linear trends along continuous variables.
#'
#' The first column of the resulting matrix is aligned to show a positive correlation
#' with the input vector and typically captures the main linear or monotonic trend.
#'
#' @param x A numeric vector representing a continuous covariate (e.g., distance or pseudotime).
#' @param df Integer. Degrees of freedom for the spline basis. Default is 3.
#'
#' @return A numeric matrix with orthonormal columns (same number of rows as `x`).
