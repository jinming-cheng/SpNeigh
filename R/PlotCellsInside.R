...
#'
#' Visualizes cells that fall within defined spatial regions (boundaries or rings),
#' typically obtained using the `GetCellsInside()` function. The cells are colored by cluster,
#' and the function offers two plotting modes: using `geom_sf()` (with fixed 1:1 aspect ratio)
#' or `geom_point()` (with more flexible layout).
#'
#' @inheritParams PlotBoundary
#' @importFrom rlang .data
#' @param cells_inside An `sf` object of cells returned by `GetCellsInside()`. Must contain `cluster` and `region_id` columns.
#' @param fixed_aspect_ratio Logical. If `TRUE`, uses `geom_sf()` to preserve spatial scale.
#'        If `FALSE`, uses `geom_point()` with extracted coordinates. Default is `TRUE`.
#'
#' @return A `ggplot` object showing the cells colored by cluster within spatial regions.
#'
#' @export
#' @examples
