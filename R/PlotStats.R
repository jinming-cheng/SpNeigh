...
#'
#' Creates a bar plot to visualize the distribution of cells inside
...
#'                    use for the y-axis.
#'                    Options are `"count"` (number of cells) or `"proportion"`
#'                    (relative fraction per region).
#'
#' @return A `ggplot2` object showing a faceted bar plot of cell statistics
#'         per region.
#'
#' @export
#' @examples
...
#'                   otherwise, a pie chart. Default is `FALSE`.
#' @param add_labels Logical. If `TRUE`, percentage labels are displayed
#'                   inside each slice. Default is `TRUE`.
#' @param label_cutoff Numeric. Proportional threshold below which labels
#'                     are hidden (e.g., `0.01` = 1%). Default is `0.01`.
#' @param label_color Character. Color of the percentage labels.
#'                    Default is `"white"`.
#' @param label_size Numeric. Text size for percentage labels. Default is `4`.
#' @param label_nudge_x Numeric. Horizontal adjustment for label positioning.
#'                      Default is `0.1`.
#'
#' @return A `ggplot2` object representing a faceted pie or donut chart
#'         per spatial region.
#'
#' @export
#' @examples
