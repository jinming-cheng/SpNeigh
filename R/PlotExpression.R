...
#'                 (based on `one_cluster` or `sub_cells`).
#'                 Default is `FALSE`.
#' @param one_cluster Optional. Cluster ID to subset cells if `sub_plot = TRUE`.
#' @param sub_cells Optional. Vector of cell IDs to include in the plot
#'                  if `sub_plot = TRUE`. If both `one_cluster` and `sub_cells`
#'                  are provided, the intersection is used.
#' @param split_by Optional. Column name in the metadata (from `data`) to
#'                 facet the plots (e.g., by `cluster`).
#' @param return_list Logical. If `TRUE`, returns a named list of individual
#'                    ggplot objects per gene.
#'                    If `FALSE` (default), plots are wrapped into
#'                    a single patchwork layout.
#' @param point_size Numeric. Size of plotted points. Default is `0.2`.
#' @param shuffle Logical. If `TRUE`, shuffles cell order before plotting.
#'                Otherwise, cells with higher expression are plotted on top.
#'                Default is `FALSE`.
#'
...
PlotExpression <- function(
    ...
    if (!identical(as.character(sp_coords$cell), colnames(exp_mat))) {
        stop(
            "Cell names in `data` and `exp_mat`",
            " must match and be in the same order."
        )
    }
    ...
    if (sub_plot) {
        if (!is.null(one_cluster)) {
            data_for_plot <- dplyr::filter(
                data_for_plot,
                .data$cluster == one_cluster
            )
        }
        if (!is.null(sub_cells)) {
            data_for_plot <- dplyr::filter(
                data_for_plot,
                .data$cell %in% sub_cells
            )
        }
    }
...
#'
#' Visualizes how the average expression of specified genes varies
#' along a spatial distance gradient.
...
#'                         spatial distance (or weights) for each cell.
#' @param genes Character vector specifying gene names to be plotted.
#'              Must match row names in `exp_mat`.
#' @param n_bins Integer. Number of bins to divide the spatial distance into.
#'               Default is `50`.
#' @param scale_method A string indicating how to scale the
#'                     average expression values across bins for each gene.
#' Options are:
#' \describe{
#'   \item{\code{"none"}}{No scaling (default). The average expression
#'                        is plotted as-is.}
#'   \item{\code{"zscore"}}{Standardize expression (mean 0, SD 1)
#'                          per gene using \code{scale()}.}
#'   \item{\code{"minmax"}}{Normalize expression to \[0, 1\] range per gene
#'                          using \code{scales::rescale()}.}
#' }
#' @param n_labels Integer. Number of axis labels to show along the
#'                 distance axis. Default is `6`.
#' @param row_gap Numeric between 0 (inclusive) and 1 (exclusive).
#'                Gap between rows (genes) in the plot. Default is `0.1`.
#' @param column_gap Numeric between 0 (inclusive) and 1 (exclusive).
#'                   Gap between columns (distance bins) in the plot.
#'                   Default is `0`.
#' @param label_x Character. Label for the x-axis.
#'                Default is "Spatial distance".
#' @param label_y Character. Label for the y-axis. Default is "Gene".
#'
#' @return A ggplot2 object displaying a heatmap of binned
#'         average gene expression across spatial distances.
#'
#' @export
#'
#' @examples
...
PlotSpatialExpression <- function(
    ...
    if (ncol(exp_mat) != length(spatial_distance)) {
        stop(
            "Number of columns in `exp_mat`",
            " must match length of `spatial_distance`."
        )
    }
    if (!all(genes %in% rownames(exp_mat))) {
        stop("One or more specified genes are not found in `exp_mat`.")
    ...
    df_long <- df %>%
        ...
        dplyr::summarise(
            avg_expression = mean(.data$expression),
    ...
    if (scale_method != "none") {
        df_long <- df_long %>%
            ...
            dplyr::mutate(avg_expression = dplyr::case_when(
                scale_method == "zscore" ~ as.numeric(
                    scale(.data$avg_expression)
                ),
                scale_method == "minmax" ~ scales::rescale(
                    .data$avg_expression,
                    to = c(0, 1)
                ),
